...
export const FLOW_NODES: FlowNodeDef[] = [
  {
    ...
    how: "Every 15 minutes Vercel calls /api/cron/daily. Schedule slots open a generation window about 6 hours before planned_publish_at, so the worker can upload a private YouTube scheduled video before the public slot.",
    connects: [
  ...
  },
  {
    ...
    connects: ["Requires YouTube connected", "Requires AI trained", "Ends in scheduled or published"],
  ...
  },
  {
    ...
    how: "All work lives in video_jobs. Status moves: queued → generating_script → generating_voice → fetching_media → editing → uploading/ready → scheduled/published/failed. The worker claims the oldest due job with an optimistic lock.",
  ...
  },
  {
    ...
    how: "videos.insert uses private + publishAt for future slots and public for immediate publish. Saves youtube_video_id/url, records upload metadata, logs usage. Temp work folder is cleaned after YouTube accepts the upload.",
