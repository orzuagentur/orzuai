...
const fetchMessagesSinceSchema = z.object({
  conversationId: z.string().uuid(),
  afterCreatedAt: z.string().min(1),
  afterMessageId: z.string().uuid().optional(),
});

/** Gap-sync: fetch messages after a cursor and advance the conversation sync pointer. */
export async function fetchConversationMessagesSinceAction(
  input: z.infer<typeof fetchMessagesSinceSchema>,
) {
  if (!hasSupabaseEnv()) {
    return {
  ...
  const parsed = fetchMessagesSinceSchema.safeParse(input);
