...
const WORKER_PATH = "/api/workers/ai-orchestration-queue";
...
export async function dispatchAiOrchestrationQueueWorker(input: {
  ...
  const deduplicationId = `ai-orchestration-drain-${input.source}-${Math.floor(Date.now() / 500)}`;
  ...
  try {
    ...
    console.error("[ai-orchestration-queue] QStash dispatch failed", error);
    return { dispatched: false };
  }
}
