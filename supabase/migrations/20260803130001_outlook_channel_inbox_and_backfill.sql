-- Treat Outlook mailbox threads like email for inbox contact dedupe.
...
BEGIN
  ...
  WITH filtered AS (
    SELECT
      ...
      CASE
        WHEN c.channel IN (
          'email'::public.messaging_channel,
          'outlook'::public.messaging_channel
        ) THEN
          lower(
            trim(
              COALESCE(
    ...
    WHERE c.business_id = p_business_id
      ...
      AND (
        ...
        OR c.channel IS DISTINCT FROM 'voice'::public.messaging_channel
      )
      AND (
        v_tsquery IS NULL
